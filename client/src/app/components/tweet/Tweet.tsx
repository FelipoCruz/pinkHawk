...
import { deleteTweet, Tweet, TweetState } from '../../../store/slices/tweet.slice';
...
const Tweet: React.FC = ({ tweetPassed }) => {
  ...
  const handleSelected = async (user: UserState, tweet: Tweet) => {
    try {
      ...
      const tweetWithNewStatus = await changeTweetStatusFromUserAPI(user.id, (tweet.status = 'queue'));
